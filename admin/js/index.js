...
{
  if($("#news_content p").length==1 && $("#news_content p").html() == "<br>")
  {
...
$(document).ready(function(){
  ...
  $("#news_content").on("paste", function (e) {
      textInit(e)
  });
  ...
  $('#chooseImage').fileupload({
    url: "./mysql/upload_img.php",
      ...
      if(lastEditRange)
      {
        ...
        $("#news_content p").html("");
        $("#news_content p").append("<img src=\""+escape(data.result)+"\" style=\"display: block; max-width: 100px; margin: 0px auto;\">").before("<p><br></p>").after("<p><br></p>")
        var range = window.getSelection();//创建range
        var obj = document.getElementsByTagName("p")
  ...
  $("#button_submit").click(function(){
    ...
    var arr = new Array();
    var img_number = $("#news_content p img").length;
    ...
    for(var i = 0;i < img_number; i++){
      src[i] = $("#news_content p img:eq("+i+")").attr("src");
      (function(i){
        setTimeout(function(){
          content = content.replace(new RegExp("<img src=\""+src[i]+"\" style=\"display: block; max-width: 100px; margin: 0px auto;\">",'g'), "[img]");
          console.log(content);
          $.post("./mysql/move_file.php",{type:'news',number:(i + 1),src:src[i]},function(result){
            arr.push(result);
    ...
    setTimeout(function(){
      arr.sort();
      var str=arr.join(",");
      $.post("./mysql/upload_data.php",{type:'news',title:title,content:content,img_id:str,number:img_number},function(result){
        alert("发送成功！");
        window.location.href = '../news-page.html?id='+result;
      });
    },1000); 
  }); 
});