...
{
  if($("#comic_content p").length==1 && $("#comic_content p").html() == "<br>")
  {
...
$(document).ready(function(){
  $("#comic_content").on("paste", function (e) {
      textInit(e)
  });
  
  $('#comic_content').keyup(function(){
    // 获取选定对象
    var selection = getSelection()
    // 设置最后光标对象
    lastEditRange = selection.getRangeAt(0)
  });
  $('#comic_content').click(function(){
    // 获取选定对象
    var selection = getSelection()
    // 设置最后光标对象
    lastEditRange = selection.getRangeAt(0)
  ...
  $('#chooseImage').fileupload({
    url: "./mysql/upload_img.php",//文件的后台接受地址
      ...
      if(lastEditRange)
      {
        ...
        $("#comic_content p").html("");
        $("#comic_content p").append("<img src=\""+escape(data.result)+"\" style=\"display: block; max-width: 100px; margin: 0px auto;\">").before("<p><br></p>").after("<p><br></p>")
        var range = window.getSelection();//创建range
        var obj = document.getElementsByTagName("p")
  ...
  $("#button_submit").click(function(){
    var title = $("#comic_title").val();
    var content = $("#comic_content").html();
    var arr = new Array();
    var img_number = $("#comic_content p img").length;
    ...
    for(var i = 0;i < img_number; i++){
      src[i] = $("#comic_content p img:eq("+i+")").attr("src");
      (function(i){
        setTimeout(function(){
          content = content.replace(new RegExp("<img src=\""+src[i]+"\" style=\"display: block; max-width: 100px; margin: 0px auto;\">",'g'), "[img]");
          console.log(content);
          $.post("./mysql/move_file.php",{type:'comic',number:(i + 1),src:src[i]},function(result){
            arr.push(result);
    ...
    setTimeout(function(){
      arr.sort();
      var str=arr.join(",");
      $.post("./mysql/upload_data.php",{type:'comic',title:title,content:content,img_id:str,number:img_number},function(result){
        alert("发送成功！");
        window.location.href = '../culture-page.html?type=comic&id='+result;
      });
    },1000); 
  }); 
});